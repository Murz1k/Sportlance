...
namespace Sportlance.WebAPI.Utilities
{
    public static class EnumUtils
    {
        ...
        {
            if (Enum.TryParse(str, out T code)) return code;
        ...
        {
            return Enum.GetNames(typeof(T)).Select((x, i) => (x, Parse<T>(x))).ToDictionary(x => x.Item2, x => x.Item1);
        }

        public static List<NamedValue<T>> GetKeyValues<T>() where T : struct
        {
            return Enum.GetNames(typeof(T)).Select(x => NamedValue.Create(x, Parse<T>(x))).ToList();
        }
